...
class CassiaDataset(BaseDataset):
    def __init__(self, data_config: munch.munchify, **kwargs: Dict[str, Any]):
        ...
        """
        super(CassiaDataset, self).__init__()
    ...
    def _load_samples(self, samples: Union[Path, List[List[Dict[str, Any]]]] = None) -> List[Dict[str, Any]]:
        ...
        if isinstance(samples, list):
            ...
            data_path = str(samples) if isinstance(samples, Path) else self.data_config.data_path
            if data_path:
                data_dirs = self._load_sample_from_multi_folders(data_path)
                for label_path in data_dirs:
    ...
    def _load_annotations(self, sample: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Loading ground-truth in a given label file.

        Arguments:
            sample: Input cassia format sample.

        Return:
            polygons: List of polygon, each with [[x1, y1], [x2, y2], ..].
        """
        annotations: Dict[int, Any] = {}
        for idx, region in enumerate(sample):
            region["polygon"] = region["location"]
            annotations[idx] = region
        return annotations
